...
- [2025-11-26] Added global cluster toggle (Mainnet Beta or Devnet), updated docs/README, and new env vars (`NEXT_PUBLIC_SOLANA_DEVNET_RPC`, `NEXT_PUBLIC_CUSDC_DEVNET_MINT`, `NEXT_PUBLIC_DEFAULT_CLUSTER`). Use devnet while `zk-elgamal-proof` remains disabled on mainnet. Next: mint CT-enabled cUSDC on devnet, wire env, and run full configure → CT transfer test there.
- [2025-11-26] Began CT helper CLI (`darkdrop-ct-service/`). CLI parses WASM proof payloads (`configure` + `transfer`) so we can push proofs into context state accounts via Rust instead of re-implementing SPL logic in JS. Added RPC bootstrap (connects to cluster using supplied keypair, validates blockhash) + sample payloads (`sample-config.json`, `sample-transfer.json`). Next: implement actual proof context state creation + CT submission and expose a simple API for the web app to call.
- [2025-11-26] Devnet CT mint (`AvV96PCeHS32crtNXqf2Dgb4BanNRCL6hhHyc6vAh6TR`) created under Token-2022 with auto-approved CT accounts. Seeded 100 cUSDC-devnet to test wallet ATA (`GRtnWRSN8LLkGV9PBnStWTo8czi4FBMjirksraLR5sBd`) and wired the mint as the default fallback so the UI works without extra env vars. Next: confirm configure + transfer succeed on devnet.
- [2025-11-26] Added `/api/proof-context` route to shell out to `darkdrop-ct-service`. Provide `DARKDROP_CT_KEYPAIR` (and optional `DARKDROP_CT_HELPER_BIN`) env vars so the API can sign proof context accounts. The endpoint writes the WASM payload to a temp file, runs the helper in `--json` mode, and returns the resulting context pubkeys + tx signatures. Next: wire `/drop/create` to call this endpoint before to building CT instructions.
