...
- [2025-11-26] Began CT helper CLI (`darkdrop-ct-service/`). CLI parses WASM proof payloads (`configure` + `transfer`) so we can push proofs into context state accounts via Rust instead of re-implementing SPL logic in JS. Next: hook the helper into Solana RPC (context account creation + CT transfer submission) and expose a simple API for the web app to call.
